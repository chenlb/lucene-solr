...
public class NettySolrClientTest {
  ...
  private static final Logger logger = LoggerFactory
      .getLogger(NettySolrClientTest.class);
  ...
  protected static class MockSolrServerHandler extends
      SimpleChannelInboundHandler<SolrProtocol.SolrRequest> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, SolrProtocol.SolrRequest solrRequest) throws Exception {
      ...
      if(errorCode != null) {
        ...
        errorCode = NettySolrException.VootooErrorCode.getErrorCode(errorCode).code;

        if(errorCode == 500 || errorCode < 100) {
          logger.info("add exception trace test");
          exceptionBody.setTrace(
              SolrException.toStr(new RuntimeException("mock trace info")));
        }
      ...
      if(responseId != null) {
        ...
        responseBodyBuilder.setBody(ByteStringer
            .wrap(responeOutput.getBuffer(), 0, responeOutput.getCount()));
  ...
  protected static class MockSolrServerChannelInitializer extends
  ...
  protected void assertNettySolrException (Throwable e, int code,
      SolrQuery solrQuery, boolean hasTrace) {
    Assert.assertTrue(e instanceof NettySolrException);
    Assert.assertEquals(((NettySolrException) e).code(), code);
    Assert.assertEquals(e.getMessage(), solrQuery.get(RequestParams.ErrorMsg));
    Assert
        .assertEquals(hasTrace, ((NettySolrException) e).getRemoteTrace() != null);
  }
  ...
  public void test_netty_client_solr_exception() {
    ...
    try {
      ...
      assertNettySolrException(e, 503, solrQuery, false);
    }
    ...
    try {
      ...
      assertNettySolrException(e, 429, solrQuery, false);
    }
    ...
    try {
      ...
      assertNettySolrException(throwable,
          NettySolrException.VootooErrorCode.UNKNOWN.code, solrQuery, true);

      NettySolrException ve = (NettySolrException) throwable;
  ...
  public void test_in_query_other_exception() {
    ...
    try {
      ...
      NettySolrException ve = (NettySolrException) e;
