...
import org.apache.solr.client.solrj.impl.netty.connect.ConnectionPool;
import org.apache.solr.client.solrj.impl.netty.connect.ConnectionPoolContext;
import org.apache.solr.client.solrj.impl.netty.connect.SimpleConnectionPool;
...
public class NettySolrClient extends SolrClient {
  ...
  private static final Logger requestLogger = LoggerFactory
      .getLogger(SolrClient.class.getPackage().getName() + ".Request");
  ...
  protected ResponseParser createRequest(SolrRequest request, String collection, ProtobufRequestSetter saveRequestSetter) throws
  ...
  protected NettySolrException createTimeoutException(long rid, int timeout, SolrParams solrParams) {
    return createTimeoutException(rid, timeout, "", solrParams);
  }

  protected NettySolrException createTimeoutException(long rid, int timeout, String tip, SolrParams solrParams) {
    NettySolrException nettySolrException = new NettySolrException(
        NettySolrException.VootooErrorCode.TIMEOUT,
        "rid=" + rid + ", [" + serverUrl() + "] is timeout=" + timeout + ", "+tip+" request=[" + solrParams + "]");
    nettySolrException.setRemoteServer(serverUrl());
    return nettySolrException;
  }
  ...
  public NamedList<Object> request(SolrRequest request, String collection) throws
    ...
    ResponseParser parser = createRequest(request, collection, protobufRequestSetter);
    ...
    try {
      ...
      if(protocolResponse.getExceptionBodyCount() > 0) {
        List<NettySolrException> solrExcs = new ArrayList<>(protocolResponse.getExceptionBodyCount());
        NettySolrException firstE = null;
        int eNum = 0;
        //has exception
        for(SolrProtocol.ExceptionBody e : protocolResponse.getExceptionBodyList()) {
          NettySolrException se = ProtobufUtil.toVootooException(e);
          se.setRemoteServer(serverUrl());
          if(se.code() != NettySolrException.ErrorCode.UNKNOWN.code) {
